...
最終更新: 2026-04-25（M3 kickoff、`docs/SEARCH_DSL.md` landed）
...
| # | モジュール | 依存 | メモ |
| --- | --- | --- | --- |
| 1 | `docs/SEARCH_DSL.md` | なし | **landed**（feat/m3-search-dsl-spec、本 PR） |
| 2 | `core::search` parser + AST | DSL 仕様書 | lex / parse / validate / AST。golden examples を `docs/SEARCH_DSL.md §10` から起こす。`core::search::{lex, parse, validate, ast}` モジュール構成。 |
| 3 | `core::search` planner + executor | parser | AST → SQL string + params の純関数。SQLite で実行する executor。FTS5 を使うのは次フェーズだが planner は FTS5 サブクエリを生成しておく |
