...
- `type:psd type:tif` = AND ではなく **OR を強制したい場合は明示**: `type:psd OR type:tif`（`type:` は通常 AND だが、同一ファイルが複数 type を持つことはないので AND は実質「結果 0 件」）。
  - **parser warning**: 同一 `type:` の暗黙 AND（複数 `type:` を空白区切り）は実質 0 件になる。loader は `warning: type-and-multi` を出す（lint と同様、stderr / json `warnings`）。
