...
最終更新: 2026-04-23（`core::rules` 本体 landed、次は `core::accepts`）
...
- `core::rules` 本体 landed（feat/m2-core-rules）: loader（forward-compat schema gate）/ applies_to（glob + specificity）/ template parser & matcher（`{prefix}` `{desc}` `{seq}` `{version}` `{ext}` `{field:}` `{date:}`、compound ext 最長一致）/ constraint evaluator（charset / casing / forbidden / reserved / length / required prefix-suffix、AND 合成）/ inheritance（full-replace override）/ evaluate 本体 + `RuleHit` trace。unit tests ~260 + §10.1–§10.6 と Codex 指摘 #1/#3/#4 を 1:1 で固定する golden fixtures。
- 次: `core::accepts`。placement lint / インポート先ランキング。本 PR の Codex レビューで洗い出された残課題（suggested_names / `{seq}` 採番 / trace の `NotApplicable` 追加 / Regex::new ホットパス化 / §4.3 escape・§4.4 mixed spec 等の golden 拡充）は別 issue で管理する想定。
...
| 2 | `core::accepts` | `.dirmeta.toml` loader + rules | `document.extra.get("accepts")` を typed にして effective_accepts 計算 / placement lint / インポート先ランキング。組み込みエイリアス（`:image`, `:video`, ...）の構成拡張子をここで確定し `docs/` に記載。 |
...
## 5. 次モジュール（`core::accepts`）kickoff 質問テンプレ

`core::accepts` 着手時に AskUserQuestion で確認する想定:

1. スコープ: `.dirmeta.toml` の `[accepts]` パース + effective_accepts 計算だけ / placement lint 本体まで / import ランキング API も含める
2. ビルトインエイリアス（`:image`, `:video`, `:audio`, `:3d`, `:doc` 等）の構成拡張子は `docs/REQUIREMENTS.md` or 新規 `docs/ACCEPTS_ALIASES.md` に記載するか
3. accepts violation の severity は rules と同じく mode（strict/warn/hint/off）に乗せるか、別軸にするか
4. 1 PR 粒度 / 既往と同じく feat branch + squash merge で良いか

旧 `core::rules` kickoff 質問は履歴参照。本ドキュメントが参考資料として kickoff で引用されることを想定している。更新漏れは即 kickoff の質問から確認する。
