...
最終更新: 2026-04-30（**M3 完了**。全 PR landed + 実機 smoke テスト済み + `@vitejs/plugin-react` v6 oxc 移行 + oxlint warning 修正。次は M4）
...
- **M4 サムネ + 外部連携 + AI + テンプレート**: **次着手**。`core::import`（accepts ランキング + rename preview の一体適用、history `Operation::Import`）/ `core::thumbnail` / `core::template` / `core::ai`。M4 import kickoff 向け sequence 統合設計メモは [`docs/M2_HANDOFF.md §5`](./M2_HANDOFF.md)
