...
現在のフェーズ: **M4 サムネ + 外部連携 + AI + テンプレート 進行中**。M1〜M3 完了。M4 は import（copy デフォルト / move opt-in、accepts ランキング、sequence 集約）→ thumbnail（WebP 256px、ffmpeg sidecar LGPL 同梱）→ template → AI（BYOK、命名/タグ/notes/配置先提案）の順に着手。進捗詳細は `docs/IMPLEMENTATION_PLAN.md §0`・§5、M4 進行中の引き継ぎは `docs/M4_HANDOFF.md`、import 設計メモは `docs/M2_HANDOFF.md §5` 参照。
...
- **タイトルバーを Tauri の OS デフォルトに任せる**（macOS では `titleBarStyle: "Overlay"` + `hiddenTitle: true` + 自前 `<TitleBar>` が canonical。理由とハマりどころは `docs/LESSONS.md §17`）
...
進捗スナップショットは [`docs/IMPLEMENTATION_PLAN.md §0`](./docs/IMPLEMENTATION_PLAN.md)、M4 進行中の引き継ぎは [`docs/M4_HANDOFF.md`](./docs/M4_HANDOFF.md)。M2_HANDOFF / M3_HANDOFF は完了アーカイブとして残置。作業一区切りごとに IMPLEMENTATION_PLAN §0 と M4_HANDOFF を更新する。
...
- [docs/M4_HANDOFF.md](./docs/M4_HANDOFF.md) — **M4 進行中の引き継ぎメモ**（kickoff 決定事項、PR 分割、import 設計メモ）
- [docs/SEARCH_DSL.md](./docs/SEARCH_DSL.md) — M3 search DSL 仕様書
- [docs/M2_HANDOFF.md](./docs/M2_HANDOFF.md) — M2 完了アーカイブ（M4 import 向け sequence 統合設計メモを §5 に保持）
- [docs/M3_HANDOFF.md](./docs/M3_HANDOFF.md) — M3 完了アーカイブ
