...
現在のフェーズ: **M1 Core data layer 進行中**。M0 Skeleton 完了、`progest-core::fs` / `progest-core::identity` / `progest-core::meta`（TOML I/O + 原子書込）が landed（PR #3/#4 merged、PR #5 in review）。`core::meta` の残タスク（`.progest/local/pending/` queue、`.dirmeta.toml` loader）は別 PR。次は `core::index` が本線。
...
**M1 Core data layer（進行中）**。M0 足場は出来上がり、`core::fs` / `core::identity` / `core::meta`（の最初のスライス）が landed。
