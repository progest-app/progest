...
現在のフェーズ: **M4 サムネ + 外部連携 + AI + テンプレート 準備中**。M1〜M3 完了。M3 では `core::search`（FTS5 trigram）+ コマンドパレット + tree/flat view（DataTable）+ ディレクトリ/ファイルインスペクター + project init UI + CLI `search`/`tag`/`view` + Tauri IPC 全実装 + shadcn/ui + Vite+ 統合。M4 は `core::import`（accepts ランキング + rename preview 一体適用）/ `core::thumbnail` / `core::template` / `core::ai` が範囲。進捗詳細は `docs/IMPLEMENTATION_PLAN.md §0`・§5、M4 import kickoff 向け sequence 統合設計メモは `docs/M2_HANDOFF.md §5` 参照。M3 完了アーカイブは `docs/M3_HANDOFF.md`。
...
進捗スナップショットは [`docs/IMPLEMENTATION_PLAN.md §0`](./docs/IMPLEMENTATION_PLAN.md)。M2_HANDOFF / M3_HANDOFF は完了アーカイブとして残置（M4 import kickoff 向け設計メモは M2_HANDOFF §5 に保持）。作業一区切りごとに IMPLEMENTATION_PLAN §0 を更新する。
